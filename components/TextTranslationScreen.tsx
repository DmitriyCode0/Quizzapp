...
import { TextTranslationQuestion, TextTranslationUserAnswer, FeedbackItem, CEFRLevel } from '../types';
...
const TextTranslationScreen: React.FC<TextTranslationScreenProps> = ({ question, onComplete, cefrLevel }) => {
  ...
  return (
    <div className="bg-slate-800 p-8 rounded-lg shadow-2xl w-full max-w-3xl animate-fade-in">
        <div className="mb-6 text-center">
            <h1 className="text-3xl font-bold text-indigo-400">Text Translation</h1>
            <p className="text-slate-300 mt-1">Translate the following text into English.</p>
        </div>

        <form ref={formRef} onSubmit={formSubmitHandler} className="flex flex-col items-center">
            <div className="w-full text-left mb-4 p-4 bg-slate-900 rounded-md border border-slate-700">
            ...
            <textarea
                ...
                placeholder="Type your full translation here..."
            ...
            {isEvaluating && (
                <div className="flex items-center gap-3 text-lg text-slate-300 mb-6">
                    ...
                    <span>Evaluating your translation...</span>
                </div>
            )}
                    ...
                    <div className="mt-4 text-left space-y-3">
                        ...
                        {evaluation.score < 100 && (
                            <div className="border-t border-slate-700 pt-3">
                                <p className="text-slate-300 text-md font-semibold">Suggested answer:</p>
                                <p className="italic text-green-300">{question.englishAnswer}</p>
                            </div>
                        )}
                    </div>
                </div>
            ...
            >
                {isAnswered ? "Finish & See Results" : "Check Answer"}
            </button>
        </form>
    </div>
  );
