...
const DiscussionScreen: React.FC<DiscussionScreenProps> = ({ prompts, title, onRestart }) => {
  ...
  const handleExportTxt = () => {
    ...
    downloadFile(content, `${title.replace(/\s+/g, '_')}.txt`, 'text/plain');
  ...
  const handleExportCsv = () => {
    ...
    downloadFile(content, `${title.replace(/\s+/g, '_')}.csv`, 'text/csv');
    setShowExportModal(false);
  };
      ...
      {showExportModal && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 animate-fade-in" onClick={() => setShowExportModal(false)}>
          <div className="bg-slate-800 rounded-lg shadow-2xl p-8 max-w-sm w-full flex flex-col gap-4" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-2xl font-bold text-center text-indigo-400 mb-2">Export Prompts</h2>
